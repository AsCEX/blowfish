...
class BlowfishMod {
	...
	public $padding = Blowfish::BLOWFISH_PADDING_RFC;
	public $mode = NULL;
	public $k = "";
	public $pt = "";
	public $ct = "";
	...
	public function encrypt($pt, $k, $iv = null){
		    $this->hexText = $pt;
		    $this->hexKey = $k;
		    $this->hexIV = $iv;
			...
			$this->mode = Blowfish::BLOWFISH_MODE_CBC;
			if(!$iv){
				$this->mode = Blowfish::BLOWFISH_MODE_EBC;
			}

		    if ($this->mode == Blowfish::BLOWFISH_MODE_CBC) {
		      $this->iv = trim($iv);
		    ...
		    if ($this->mode == Blowfish::BLOWFISH_MODE_CBC) {
		      $this->padding = Blowfish::BLOWFISH_PADDING_ZERO;
		    } else {
		      $this->padding = Blowfish::BLOWFISH_PADDING_NONE;
		    }

			$bfish = Blowfish::encrypt($this->pt, $this->k, $this->mode, $this->padding, $this->iv);

			$this->ciphertext = $bfish['ciphertext'];

			$time = $bfish['end'] - $bfish['start'];

			$plain_text = strtoupper(reset(unpack("H".strlen($this->hexText), $this->pt)));
			$encryption_key = strtoupper(reset(unpack("H".strlen($this->hexKey), $this->k)));
			$cipher_text =  strtoupper(reset(unpack("H".(strlen($this->hexText)+$this->padding_cbc), $this->ciphertext)));
	...
	public function decrypt($ct, $k, $iv=null){ 
		    ...
		    $b = new Blowfish();
			$bfish = $b->decrypt($this->ct, $this->k, $this->mode, $this->padding, $this->iv);
